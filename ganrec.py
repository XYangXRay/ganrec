...
#     else:
#         conv = tf.compat.v1.layers.conv2d(x, conv_nb, [conv_size, conv_size], padding='SAME', activation=tf.nn.relu,
#                                 kernel_initializer=net_init, name=name, reuse=tf.AUTO_REUSE)
#     conv = tf.compat.v1.layers.batch_normalization(conv, name = name2, reuse=tf.AUTO_REUSE)
...
#
#     fc4 = tf.compat.v1.layers.flatten(conv4)
...
#     conv1 = tf.reshape(fc1a, shape=[-1, px // 8, px // 8, 1])
#
#     conv1 = tf.compat.v1.layers.conv2d_transpose(conv1, conv_nb, [conv_size, conv_size], padding='SAME',
#                                        activation=tf.nn.relu, kernel_initializer=net_init, name='conv1',
#                                        reuse=tf.compat.v1.AUTO_REUSE)
#     conv1 = tf.compat.v1.layers.conv2d_transpose(conv1, conv_nb, [conv_size, conv_size], padding='SAME',
#                                       strides=(8, 8),
#                                       activation=tf.nn.relu, kernel_initializer=net_init,
#                                       name='conv1a',reuse=tf.compat.v1.AUTO_REUSE)
#
#     conv2 = tf.reshape(fc2a, shape=[-1, px // 4, px // 4, 1])
#     conv2 = tf.compat.v1.layers.conv2d_transpose(conv2, conv_nb, [conv_size, conv_size], padding='SAME',
#                                        activation=tf.nn.relu, kernel_initializer=net_init, name='conv2',
#                                        reuse=tf.compat.v1.AUTO_REUSE)
#     conv2 = tf.compat.v1.layers.conv2d_transpose(conv2, conv_nb, [conv_size, conv_size], padding='SAME',
#                                       strides=(4, 4),
#                                       activation=tf.nn.relu, kernel_initializer=net_init,
#                                       name='conv2a',reuse=tf.compat.v1.AUTO_REUSE)
#     conv3 = tf.reshape(fc3a, shape=[-1, px // 2, px // 2, 1])
#     conv3 = tf.compat.v1.layers.conv2d_transpose(conv3, conv_nb, [conv_size, conv_size], padding='SAME',
#                                        activation=tf.nn.relu, kernel_initializer=net_init, name='conv3',
#                                        reuse=tf.compat.v1.AUTO_REUSE)
#     conv3 = tf.compat.v1.layers.conv2d_transpose(conv3, conv_nb, [conv_size, conv_size], padding='SAME',
#                                       strides=(2, 2),
#                                       activation=tf.nn.relu, kernel_initializer=net_init,
#                                       name='conv3a',reuse=tf.compat.v1.AUTO_REUSE)
#     conv4 = tf.reshape(fc4a, shape=[-1, px, px, 1])
#     conv4 = tf.compat.v1.layers.conv2d(conv4, conv_nb, [conv_size, conv_size], padding='SAME',
#                                       activation=tf.nn.relu, kernel_initializer=net_init, name='conv4',
#                                       reuse=tf.compat.v1.AUTO_REUSE)
#
#     cc = tf.concat([conv1, conv2, conv3, conv4], axis=3)
#     conv5 = tf.compat.v1.layers.conv2d(cc, conv_nb, [conv_size, conv_size], padding='SAME',
#                                       activation=tf.nn.relu, kernel_initializer=net_init, name='conv5',
#                                       reuse=tf.compat.v1.AUTO_REUSE)
#     conv5 = tf.compat.v1.layers.conv2d(conv5, conv_nb//8, [conv_size, conv_size], padding='SAME',
#                                        activation=tf.nn.relu, kernel_initializer=net_init, name='conv6',
#                                        reuse=tf.compat.v1.AUTO_REUSE)
#     conv6 = tf.compat.v1.layers.conv2d(conv5, 1, [conv_size, conv_size], padding='SAME',
...
def discriminator(x, reuse = False, conv_nb = 32, conv_size = 3, dropout = 0.25):
    with tf.compat.v1.variable_scope('discriminator', reuse = reuse):
        ...
        # x = tf.compat.v1.layers.conv2d(x, 1, [conv_size, conv_size], strides=(1, 1), padding='same',
        ...
        #
        # x = tf.compat.v1.layers.conv2d(x, conv_nb, [conv_size, conv_size], padding='same',
        #                     activation=tf.nn.leaky_relu, kernel_initializer=dis_init,
        #                     name='convd1')
        # # x = tf.compat.v1.layers.conv2d(x, conv_nb, [conv_size, conv_size],
        # #                      activation=tf.nn.relu, kernel_initializer=dis_init,
        # #                      name='convd1a')
        # x = tf.compat.v1.layers.average_pooling2d(x, 2, 2)
        # x = tf.compat.v1.layers.conv2d(x, conv_nb*2, [conv_size+2, conv_size+2], padding='same',
        #                     activation=tf.nn.leaky_relu, kernel_initializer=dis_init,
        #                     name='convd2')
        # # x = tf.compat.v1.layers.conv2d(x, conv_nb * 2, [conv_size, conv_size],
        # #                      activation=tf.nn.relu, kernel_initializer=dis_init,
        # #                      name='convd2a')
        # x = tf.compat.v1.layers.average_pooling2d(x, 2, 2)
        # x = tf.compat.v1.layers.conv2d(x, conv_nb*4, [conv_size+4, conv_size+4], padding='same',
        #                     activation=tf.nn.leaky_relu, kernel_initializer=dis_init,
        #                     name='convd3')
        # # x = tf.compat.v1.layers.conv2d(x, conv_nb * 4, [conv_size, conv_size],
        # #                      activation=tf.nn.relu, kernel_initializer=dis_init,
        # #                      name='convd3a')
        # x = tf.compat.v1.layers.average_pooling2d(x, 2, 2)
        # x = tf.compat.v1.layers.conv2d(x, conv_nb * 8, [conv_size + 4, conv_size + 4], padding='same',
        #                      activation=tf.nn.leaky_relu, kernel_initializer=dis_init,
        #                      name='convd4')
        ...
        # x = tf.compat.v1.layers.dense(x, 1024, activation=tf.nn.tanh, use_bias=True,
        ...
        # x = tf.compat.v1.layers.dense(x, 2, kernel_initializer=dis_init, name='fc2')
...
#         convd4 = conv2d_norm(convd3, conv_nb * 8, conv_size, (2, 2), dis_init, 'convd4', 'bnconvd4')
#         convd5 = conv2d_norm(convd4, 1, conv_size, (1, 1), dis_init, 'convd5', 'bnconvd5')
#         fcd1 = tf.compat.v1.layers.flatten(convd5)
...
def rec_cost(prj, ang, save_wpath, log_path, init_wpath = None, **kwargs):
    ...
    # print prj.shape
    # loss_op = tf.divide(1, tf.reduce_mean(tfa.image.ssim(Y, pred, max_val=1)))
